---
name: graphify-patched
description: "Project-local graphify with collision-safe IDs, Test->Prod direction rule, file-node format consistency, and concept-to-code linking. Use INSTEAD of global /graphify in this repo."
trigger: /graphify-patched
---

# /graphify (project-local override)

Project-local fork of the global graphify skill. Patches the subagent extraction prompt with four fixes uncovered while graphifying this repo:

- **A** — Node IDs use `{parent_dir}_{stem}_{entity}` to disambiguate symbols across packages with the same filename (e.g. `httpserver_server_newserver` vs `ipc_server_newserver`).
- **B1** — Explicit `calls` direction rule: tests/benchmarks/examples are always callers, never callees.
- **C** — File-container nodes use the same `{parent_dir}_{stem}_{ext}` format. No absolute paths. AST and semantic extractors must produce identical IDs for the same file.
- **D** — Doc/task/decision nodes that name a code symbol must emit a `describes` edge to that symbol's node.

Everything else matches the global skill at `~/.claude/skills/graphify/SKILL.md`.
...
Code files: focus on semantic edges AST cannot find (call relationships, shared data, arch patterns).
  ...
  EDGE DIRECTION RULE (CRITICAL): for `calls` relationships, the edge MUST go from caller to callee. Test/Benchmark/Example/Fuzz functions ALWAYS call production code, never the reverse. If you see `func TestFoo` invoking `Bar()`, the edge is `TestFoo --calls--> Bar`, never `Bar --calls--> TestFoo`. Same rule for setup/init code: a constructor referenced inside a test fixture is the callee, not the caller. Apply this rule before emitting any edge whose source label starts with `Test`, `Benchmark`, `Example`, or `Fuzz`.
Doc/paper files: extract named concepts, entities, citations. Also extract rationale — sections that explain WHY a decision was made, trade-offs chosen, or design intent. These become nodes with `rationale_for` edges pointing to the concept they explain.
  CONCEPT-TO-CODE LINKING: if a doc/task/decision node references a specific code symbol (function, type, file, package) by name, emit a `describes` edge from the doc node to the code node using the SAME ID format the AST extractor uses (see Node ID format below). This anchors floating concepts to their implementation and prevents documentation islands.
...
Node ID format: lowercase, only `[a-z0-9_]`, no dots or slashes. Format: `{parent_dir}_{stem}_{entity}` where parent_dir is the immediate parent directory of the file, stem is the filename without extension, and entity is the symbol name — all normalized (lowercase, non-alphanumeric chars replaced with `_`). Example: `internal/httpserver/server.go` + `NewServer` → `httpserver_server_newserver`; `internal/ipc/server.go` + `NewServer` → `ipc_server_newserver`. The parent_dir prefix is REQUIRED to disambiguate symbols that share a filename basename across packages (e.g. multiple `server.go`, `sanitize.go`, `main.go`). This must match the ID the AST extractor generates so cross-references between code and semantic nodes connect correctly. CRITICAL: never append chunk numbers, sequence numbers, or any suffix to an ID (no `_c1`, `_c2`, `_chunk2`, etc.). Never use absolute paths in IDs. IDs must be deterministic from `parent_dir + stem + label` alone — the same entity must always produce the same ID regardless of which chunk processes it.

File-container nodes (one node per source file, used for `contains` edges): use `{parent_dir}_{stem}_{ext}` with the same normalization. Example: `internal/ipc/sanitize.go` → `ipc_sanitize_go`. Do NOT use absolute paths like `home_user_project_internal_ipc_sanitize_go`. Do NOT emit two nodes for the same file under different ID schemes — the AST extractor and semantic extractor MUST produce identical IDs for the same physical file.

Output exactly this JSON (no other text):
{"nodes":[{"id":"auth_session_validatetoken","label":"Human Readable Name","file_type":"code|document|paper|image","source_file":"relative/path","source_location":null,"source_url":null,"captured_at":null,"author":null,"contributor":null}],"edges":[{"source":"node_id","target":"node_id","relation":"calls|implements|references|cites|conceptually_related_to|shares_data_with|semantically_similar_to|rationale_for|describes","confidence":"EXTRACTED|INFERRED|AMBIGUOUS","confidence_score":1.0,"source_file":"relative/path","source_location":null,"weight":1.0}],"hyperedges":[{"id":"snake_case_id","label":"Human Readable Label","nodes":["node_id1","node_id2","node_id3"],"relation":"participate_in|implement|form","confidence":"EXTRACTED|INFERRED","confidence_score":0.75,"source_file":"relative/path"}],"input_tokens":0,"output_tokens":0}
```
